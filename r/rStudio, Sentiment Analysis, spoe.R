...
party_raw <- readLines('data/euwahl19-parteiprogramme/txt/clean_text-spoe.body.txt')
...
head(negative_sentiments$token,50000)

party_df %>% 
  filter(!is.na(sentiment_pos)) %>% 
  select(token) -> positive_sentiments

head(positive_sentiments$token, 50000)
